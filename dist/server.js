...
const azure_storage_helper_1 = require("./utils/azure-storage-helper");
const azure_storage_1 = require("./config/azure-storage");
const azure_storage_helper_meat_1 = require("./utils/azure-storage-helper-meat");
...
const UPLOAD_DIR = path_1.default.join(process.env.HOME || "", "Desktop", "imlocl-uploads");
...
// Multer setup
const storage = multer_1.default.memoryStorage(); // Store in memory instead of disk
const upload = (0, multer_1.default)({
    storage,
    limits: { fileSize: 5 * 1024 * 1024 }, // 5MB
});
(0, azure_storage_1.ensureContainerExists)().catch(console.error);
...
async function findOptimalPartner(partners, _orderId) {
    if (!partners.length) {
        throw new Error("No delivery partners available");
    }
    return partners[0];
}
async function notifyPartner(_partnerId, _delivery) {
    console.log("notifyPartner placeholder called");
}
function getDeliveryStatus(_deliveryId) {
    return null;
}
// ----------------------
// helpers
...
function roleFromPartnerType(partnerType) {
    const pt = partnerType.toLowerCase();
    if (["laundry", "laundry_partner", "laundry_shop"].includes(pt))
        return client_1.PartnerRole.LAUNDRY_PARTNER;
    if (["meat_store", "meat", "meatshop", "meat_shop", "butcher"].includes(pt))
        return client_1.PartnerRole.MEAT_PARTNER;
    if (["organic", "organic_store", "organic-shop", "organicshop", "kirana", "grocery_organic"].includes(pt)) {
        return client_1.PartnerRole.ORGANIC_PARTNER;
    }
    if (["tailor", "stitching", "designer"].includes(pt))
        return client_1.PartnerRole.TAILOR;
    if (["cook", "cooking", "chef"].includes(pt))
        return client_1.PartnerRole.COOK;
    if (["delivery", "delivery_partner", "driver"].includes(pt))
        return client_1.PartnerRole.DELIVERY;
...
async function requireMeatPartnerApproved(req) {
    const auth = await authRequired(req);
    if (!auth.ok)
        return auth;
    const partner = auth.partner;
    if (partner.role !== client_1.PartnerRole.MEAT_PARTNER) {
        return { ok: false, status: 403, message: "Only Meat Partner can access this." };
    }
    if (partner.status !== client_1.PartnerStatus.APPROVED) {
        return { ok: false, status: 403, message: "Account under review. Not approved yet." };
    }
    return { ok: true, partner };
}
async function ensureMeatShop(partnerId) {
...
// ORGANIC helpers
// ----------------------
async function requireOrganicPartnerApproved(req) {
    const auth = await authRequired(req);
    if (!auth.ok)
        return auth;
    const partner = auth.partner;
    if (partner.role !== client_1.PartnerRole.ORGANIC_PARTNER) {
        return { ok: false, status: 403, message: "Only Organic Partner can access this." };
    }
    if (partner.status !== client_1.PartnerStatus.APPROVED) {
        return { ok: false, status: 403, message: "Account under review. Not approved yet." };
    }
...
async function ensureOrganicShop(partnerId) {
    const partner = await prisma.partner.findUnique({ where: { id: partnerId } });
    ...
    try {
        const shops = await prisma.meatShop.findMany({
            where: {
                isOpen: true,
                partner: {
                    role: client_1.PartnerRole.MEAT_PARTNER,
                    status: client_1.PartnerStatus.APPROVED,
                    isActive: true,
                },
            },
            include: {
...
app.get("/api/public/meatshops/:id", async (req, res) => {
    try {
        ...
        const result = {
            ...
            partner: {
                id: shop.partner?.id,
                fullName: shop.partner?.fullName,
                businessName: shop.partner?.businessName,
            },
    ...
    try {
        const shops = await prisma.organicShop.findMany({
            where: {
                isOpen: true,
                partner: {
                    role: client_1.PartnerRole.ORGANIC_PARTNER,
                    status: client_1.PartnerStatus.APPROVED,
                    isActive: true,
                },
            },
...
app.get("/api/public/organicstores/:id", async (req, res) => {
    try {
        ...
        if (!shop)
            return res.status(404).json({ message: "Organic store not found" });
        return res.json({
            shop: {
                id: shop.id,
                shopName: shop.shopName,
    ...
    try {
        ...
        const created = await prisma.partner.create({
            data: {
                ...
                fullName,
                role,
                status: client_1.PartnerStatus.PENDING,
                email,
                businessName,
                address,
                city,
...
app.post("/api/auth/login", async (req, res) => {
    try {
        ...
        if (!partner)
            return res.status(404).json({ message: "Partner not found. Please register." });
    ...
    try {
        ...
        if (req.file) {
            try {
                ...
                const uploadResult = await (0, azure_storage_helper_meat_1.uploadMeatItemImage)(req.file.buffer, shopName, name, req.file.originalname);
...
app.put("/api/meatshop/items/:id", async (req, res) => {
    try {
        ...
        if (req.body.name !== undefined)
            data.name = s(req.body.name);
        if (req.body.unit !== undefined)
            data.unit = s(req.body.unit);
        if (req.body.price !== undefined)
            data.price = Number(req.body.price);
        if (req.body.minQty !== undefined)
            data.minQty = asFloat(req.body.minQty);
        if (req.body.stepQty !== undefined)
            data.stepQty = asFloat(req.body.stepQty);
        if (req.body.inStock !== undefined)
            data.inStock = Boolean(req.body.inStock);
...
app.post("/api/deliveries", async (req, res) => {
    try {
        ...
        if (!gate.ok)
            return res.status(gate.status).json({ message: gate.message });
        const { orderId, customerName, customerPhone, customerAddress, items, totalAmount, estimatedPickupTime, } = req.body;
        ...
        const availablePartner = await prisma.deliveryPartner.findFirst({
            where: {
                ...
                partner: {
                    isActive: true,
                    status: client_1.PartnerStatus.APPROVED,
                    role: client_1.PartnerRole.DELIVERY,
                },
            },
            include: {
                partner: true,
...
app.post("/api/delivery-partners/register", async (req, res) => {
    try {
        const { fullName, phone, email, address, city, drivingLicense, vehicleNumber, vehicleType, vehicleModel, } = req.body;
...
app.post("/api/meatshop/items/:id/image", upload.single("image"), async (req, res) => {
    try {
        ...
        const shopName = shop.shopName;
        const uploadResult = await (0, azure_storage_helper_meat_1.uploadMeatItemImage)(req.file.buffer, shopName, existing.name, req.file.originalname);
...
app.post("/api/meatshop/items/:id/images", upload.array("images", 5), async (req, res) => {
    try {
        ...
        const uploadResults = await (0, azure_storage_helper_meat_1.uploadMultipleMeatItemImages)(files, shop.shopName, existing.name);
...
async function main() {
    ...
    (0, azure_storage_1.ensureContainerExists)()
...
app.get("/api/public/designers", async (_req, res) => {
    try {
        const designers = await prisma.partner.findMany({
            where: {
                role: { in: [client_1.PartnerRole.TAILOR] },
                status: client_1.PartnerStatus.APPROVED,
                isActive: true,
            },
            include: {
...
app.get("/api/public/designers/:id", async (req, res) => {
    try {
        ...
        const categories = designer.designerProfile?.categories?.map((cat) => ({
            id: cat.id,
            name: cat.name,
...
app.get("/api/designer/profile", async (req, res) => {
    try {
        ...
        if (auth.partner.role !== client_1.PartnerRole.TAILOR) {
...
app.post("/api/designer/items", async (req, res) => {
    try {
        ...
        const { subcategoryId, name, description, price, discountPrice, currency = "₹", images = [], videos = [], availability, sizes = [], measurements, deliveryTime, customizationTime, } = req.body;
...
app.put("/api/designer/items/:id", async (req, res) => {
    try {
        ...
        const { name, description, price, discountPrice, images, videos, availability, sizes, measurements, deliveryTime, customizationTime, isActive, } = req.body;
        // Verify ownership
        const item = await prisma.designerItem.findFirst({
            where: {
                id: itemId,
...
app.post("/api/designer/items/:id/images", upload.array("images", 8), async (req, res) => {
    try {
        ...
        // Upload to Azure Blob Storage
        const uploadResults = await (0, azure_storage_helper_1.uploadMultipleToAzure)(files, businessName, categoryName, subcategoryName, itemName);
        const imageUrls = uploadResults.map(result => result.url);
...
app.put("/api/partner/profile", async (req, res) => {
    try {
        ...
        if (!auth.ok)
            return res.status(auth.status).json({ message: auth.message });
        const { fullName, email, businessName, address, city, experience, } = req.body;
        ...
        if (auth.partner.role === client_1.PartnerRole.DELIVERY) {
